...
export function generateWorkout(args) {
    ...
    exer = exer.filter((key) => exercises[key].meta.environment !== "home"); // remove home exercises
    let includedTracker = [];
    let numSets = 5;
    let listOfMuscles;
    ...
    if (workout === "individual") {
        ...
        listOfMuscles = WORKOUTS[workout][muscles[0]]; // get the muscles for the selected workout
    }

    listOfMuscles = new Set(shuffleArray(listOfMuscles)); // shuffle the muscles
